    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (input.equals("1")) { // CREATE A NEW ITEM
                ...
                } else if (input.equals("3")) { // create bike
                    System.out.print("Enter the make of the bike: ");
                    String make = scanner.nextLine();
                    System.out.print("Enter the year of the bike: ");
                    int year = Integer.parseInt(scanner.nextLine());
                    System.out.print("Enter the license plate of the bike: ");
                    String licensePlate = scanner.nextLine();
                    System.out.println("What is the cost? ");
                    double cost = Double.parseDouble(scanner.nextLine());

                    Bike newBike = new Bike(licensePlate, make, year);
                    newBike.setCost(cost);
                    bikes.add(newBike);
                ...
                // get user info
                Client client = new Client(generateUniqueClientID(), "John", "No info", "No info",
                        Membership.REGULAR); // handles
                // errors
                // //change
                // later
                // maybe
                ...
                while (true) {
                    ...
                    if (input.equals("1")) {
                        for (Vacation vacation : vacations) {
                            System.out.println(
                                    "|" + vacation.getCountry() + "|" + vacation.getCity() + "|"
                                            + vacation.getSeason()
                        ...
                        for (Bike bike : bikes) {
                            System.out.println(
                                    "|" + bike.getYear() + "|" + bike.getMake() + "|" + bike.getCost() + "| ENTER ("
                                            + index + ")");
                            index += 1;
                        }
                        System.out.println("Enter which Bike you want to book. \n");
                        index = Integer.parseInt(scanner.nextLine());
                        services.add(bikes.get(index));
                    }
                    ...
                    if (client.getClientID().equals(input)) {
                        ...
                        for (Booking booking : client.getBookings()) {
                            System.out.println("BookingID: " + booking.getBookingID() + " Items: "
                                    + booking.getServices()
                                    + " From-to: " + booking.getStartDate() + "-" + booking.getEndDate()
                                    + " Price: "
